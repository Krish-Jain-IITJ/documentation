...
from .utils import cstr2str, date_filename, fopen, hhmm_to_time, rotate_file, secs_to_time, time_to_secs
...
class Config(object):
    ...
    def from_bytes(data):
        ...
        conf._wifi_name = cstr2str(wifi_name)
        conf._wifi_pass = cstr2str(wifi_pass)
        try:
        ...
        conf._version = cstr2str(version)
        return conf
...
class Picture(object):
    ...
    def from_bytes(data):
        ...
        pic = Picture()
        pic._size = size
        pic._time = secs_to_time(time_ms // 1000)
        pic._path = cstr2str(path)
...
class PictureListItem(object):
    ...
    def iter_from_bytes(data):
        ...
        for i in range(0, len(data), PictureListItem._LEN):
            ...
            yield PictureListItem(size, cstr2str(path))
        return
...
class RecordListItem(object):
    ...
    def iter_from_bytes(data):
        ...
        for i in range(0, len(data), RecordListItem._LEN):
            ...
            yield RecordListItem(start, tlen, cstr2str(path))
        return
...
class RecordPlan(object):
    ...
    def __init__(self, active=None, active_days=None, start_time=None,
        ...
        self._start_time = secs_to_time(def_stime)
        self._end_time = secs_to_time(def_etime)
        ...
        if start_time != None:
            if type(start_time) == int:
                self._start_time = secs_to_time(start_time)
            else:
                self._start_time = hhmm_to_time(start_time)
        if end_time:
            if type(end_time) == int:
                self._end_time = secs_to_time(end_time)
            else:
                self._end_time = hhmm_to_time(end_time)
    ...
    def to_bytes(self):
        ...
        return struct.pack(
            ...
            time_to_secs(self._start_time),
            time_to_secs(self._end_time),
            self._max_duration)
