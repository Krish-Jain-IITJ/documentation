...
# ── PID Altitude Hold API ────────────────────────────────────────────────────
# These endpoints talk to the altitude_hold node via ROS topics.
# The node must be running: ros2 run pluto_dashboard altitude_hold

from std_msgs.msg import Bool as BoolMsg, Float32, String as StringMsg

_pid_enable_pub  = None
_pid_target_pub  = None
_pid_gains_pub   = None
_pid_state_cache = {}

def _init_pid_publishers():
    global _pid_enable_pub, _pid_target_pub, _pid_gains_pub
    if _pid_enable_pub is None:
        _pid_enable_pub = node.create_publisher(BoolMsg,    '/pid/enable',     10)
        _pid_target_pub = node.create_publisher(Float32,    '/pid/target_alt', 10)
        _pid_gains_pub  = node.create_publisher(StringMsg,  '/pid/gains',      10)
        # Subscribe to state
        node.create_subscription(StringMsg, '/pid/state', _pid_state_cb, 10)

def _pid_state_cb(msg):
    global _pid_state_cache
    import json
    try:
        _pid_state_cache = json.loads(msg.data)
    except Exception:
        pass

@app.route('/api/pid/enable', methods=['POST'])
def api_pid_enable():
    data = request.get_json(silent=True) or {}
    target = float(data.get('target_alt', 1.0))
    try:
        _init_pid_publishers()
        # Set target first
        t_msg = Float32(); t_msg.data = target
        _pid_target_pub.publish(t_msg)
        # Enable
        e_msg = BoolMsg(); e_msg.data = True
        _pid_enable_pub.publish(e_msg)
        return jsonify({'ok': True})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500

@app.route('/api/pid/disable', methods=['POST'])
def api_pid_disable():
    try:
        _init_pid_publishers()
        e_msg = BoolMsg(); e_msg.data = False
        _pid_enable_pub.publish(e_msg)
        return jsonify({'ok': True})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500

@app.route('/api/pid/target', methods=['POST'])
def api_pid_target():
    data = request.get_json(silent=True) or {}
    target = float(data.get('target_alt', 1.0))
    try:
...
def api_pid_gains():
    ...
    try:
        _init_pid_publishers()
        g_msg = StringMsg()
        g_msg.data = _json.dumps({
            'kp': float(data.get('kp', 120)),
            'ki': float(data.get('ki', 15)),
            'kd': float(data.get('kd', 80)),
        })
        _pid_gains_pub.publish(g_msg)
        return jsonify({'ok': True})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500

@app.route('/api/pid/state')
def api_pid_state():
    return jsonify(_pid_state_cache if _pid_state_cache else {
        'enabled': False, 'target_alt': 1.0, 'current_alt': 0.0,
        'error': 0.0, 'roll_deg': 0.0, 'pitch_deg': 0.0,
        'kp': 120.0, 'ki': 15.0, 'kd': 80.0,
    })
